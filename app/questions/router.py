...
async def add_questions_in_db(grades: list = Query(description=f"Грейды: {all_grades}"),
                              topics: list = Query(description=f"Доступные технологии: {all_topics}")):
...
@router.get("/get_questios")
async def get_questions(background_tasks: BackgroundTasks,
                        grades: list = Query(
                            description=f"Грейды: {all_grades}"),
                        topics: list = Query(
                            description=f"Доступные технологии: {all_topics}"),
                        period_time: int = Query(
                            default=180, description="Периодичность указывается в минутах"),
    ...
    background_tasks.add_task(
        periodic_send_email, email, period_time, all_questions)
    return {"status": "success",
